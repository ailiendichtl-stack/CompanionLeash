...
registerForEvent("onDraw", function()
  ...
  if Triggers and ImGui.CollapsingHeader("Ihre eigenen Barks") then
    ...
    ImGui.TextWrapped("Was sie von selbst sagt. Wird beim ersten Auftreten nach "
                      .. "fremde_zeilen.txt geschrieben, weil der Log rotiert.")
    ImGui.Text(string.format("%d verschiedene", f.anzahl or 0))
    for text, e in pairs(f.liste or {}) do
      ImGui.TextDisabled(string.format("%2dx  %.1fs  %s", e.anzahl, e.dauer, text))
    end
  end
  ...
  if Triggers and ImGui.CollapsingHeader("Heilung", ImGuiTreeNodeFlags.DefaultOpen) then
    ...
    local w = Triggers.Status().wunde or {}
    local wa = ImGui.Checkbox(string.format("Wundeffekt loesen ab %.0f%%##wu",
                                            w.abHp or 80), w.an)
    if wa ~= w.an then Triggers.Setzen("wunde", wa) end
    ImGui.Text(string.format("verwundet %s   geloest %d von %d Versuchen",
                             tostring(w.hat), w.entfernt or 0, w.versuche or 0))
    if w.fehler then ImGui.TextDisabled("   " .. w.fehler) end
    ImGui.TextWrapped("Der gebueckte Gang ist ein Statuseffekt, keine Animation - deshalb "
                      .. "half weder volle Gesundheit noch die Lage, und deshalb loeste "
                      .. "ihn nur ein Respawn.")
  end
  ...
  if Triggers and ImGui.CollapsingHeader("Begegnung", ImGuiTreeNodeFlags.DefaultOpen) then
    local b = Triggers.Status().begegnung or {}
    if not b.offen then
      ImGui.TextDisabled(string.format("keine offen   (bisher %d)", b.gen or 0))
    else
      ImGui.Text(string.format("Nr %d  offen seit %.0f s  %d Welle(n)",
                               b.gen or 0, b.seit or 0, b.wellen or 0))
      ImGui.Text(string.format("Ruhe %.1f s   laengste %.1f s   fremde Stimme %s",
                               b.ruhe or 0, b.ruheLaengste or 0, tostring(b.fremd)))
    end
    ImGui.TextWrapped("Laeuft mit und sperrt NICHTS. Aus den Ruhezeiten im Log waehlen "
  ...
  if Triggers and ImGui.CollapsingHeader("Aufhelfen", ImGuiTreeNodeFlags.DefaultOpen) then
    ...
    ImGui.SameLine()
    ImGui.Text(string.format("liegt %s   geholfen %d",
                             tostring(l.hat), l.geholfen or 0))
    if l.fehler then ImGui.TextDisabled("   " .. l.fehler) end
    ImGui.TextWrapped("NCAs Menue ist im Kampf gesperrt, und der Versuch das zu umgehen "
                      .. "hat es zerlegt - wieder zurueckgenommen. Dieser Knopf tut "
                      .. "dasselbe, ohne NCA anzufassen.")
  end
  ...
  if Triggers and ImGui.CollapsingHeader("Ort", ImGuiTreeNodeFlags.DefaultOpen) then
    ...
    ImGui.Text(string.format("Stunde laut NCA: %s",
                             tostring(Triggers.Status().stunde)))
  ...
  if Triggers and ImGui.CollapsingHeader("Haltung", ImGuiTreeNodeFlags.DefaultOpen) then
    ...
    ImGui.Text(string.format("Lagewechsel %d   aufgefrischt %d",
                             st.gesetzt, st.aufgefrischt))
    ImGui.TextDisabled("   Stealth = tiefe Hocke, Relaxed = normal. Im Kampf gehoert die "
                       .. "Lage der KI.")
  end
